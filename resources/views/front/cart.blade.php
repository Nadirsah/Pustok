                        ...
                        <div class="cart-table table-responsive mb--40">
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="pro-remove"><a class="deleteCard" data-id="{{ $item->id }}"><i
                                        ...
                                        <td class="pro-title"><a
                                                href="{{route('product-detail',$item->get_products->id)}}">{!!$item->get_products->title!!}</a>
                                        </td>
                                        <td class="pro-price"><span>{{$item->get_products->price}} azn</span></td>
                                        <td class="pro-price"><span>{{$item->quantity}}</span></td>
                                        <td class="pro-subtotal"><span
                                                id="total">{{($item->get_products->price)*($item->quantity)}} azn</span>
                                        </td>
                                    </tr>
                                    @endforeach
                                    <!-- Product Row -->
                ...
                <div class="col-lg-6 col-12 d-flex">
                    <div class="cart-summary">
                        <div class="cart-summary-wrap">
                            ...
                            <p>{{__('cart.sub')}} {{__('cart.total')}} <span class="text-primary" id="totalcart"></span>
                            </p>
                            <h2>{{__('cart.grand')}} {{__('cart.total')}} <span class="text-primary">{{$totalPrice}}
                                    azn</span></h2>
                        </div>
                        <div class="cart-summary-button">
                            @if($totalPrice!==0)
                            <a href="{{route('checkout')}}"
                                class="checkout-btn c-btn btn--primary">{{__('cart.checkout')}}</a>
                            @else
